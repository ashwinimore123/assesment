...
class HomeController extends Controller{
  ...
  public function global_order_id( Request $request)
    {
             /*for update global order session*/
  ...
  public function  Add_new_globalorder( Request $request)
  {
    ...
    $global_order_id=$globalOrder->global_order_id;
 ...
 $carts=Cart::where("created_by",$user_id)->where("global_order_id",$global_order_id)->where("order_id",0)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->where('is_split',0)->with('product')->with('cartmodifier')->with('globalorder')->get();
  ...
  $view = View::make('totalammount')->with('carts',$carts)->with('globalOrder',$globalOrder);
 ...
 public function ALLExisting_Order(Request $request)
  {
    $table_id_session = $this->getTableSession();
    $branch_session = $this->BranchSession();
    $business_session = $this->putBusinessSession();
    ...
    $view = View::make('all_existing_orders')->with('globalOrder',$globalOrder);
    $sections = $view->renderSections();
...
public function Discount(Request $request)
  {
    ...
    /* $carts= Cart::where("created_by",$user_id)->where("order_id",0)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->update(['global_order_id'=>$globalOrder->global_order_id]);*/
...
*/
    ...
    public function Post_globalorder_splitproduct (Request $request)
  {
    ...
    $globalOrder->save();
     ...
     $carts= Cart::where('is_split',1)->where("created_by",$user_id)->where("order_id",0)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->update(['order_id'=>$Order->order_id]);

   
    $carts=Cart::where("created_by",$user_id)->where("order_id",0)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->where('is_split',0)->with('product')->with('cartmodifier')->with('globalorder')->get();



   $globalOrder=Cart::where('created_by',$user_id)->where("order_id",0)->where('created_by',$user_id)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->where('is_split',0)->with('product')->with('cartmodifier')->with('globalorder')->first();
  ...
  else
  {
    ...
    /*$global_order_id=$this->getGlobalOrderSession()->flush(0);*/
    
     /*$request->session()->forget($this->getGlobalOrderSession(0));*/
...
/*for couster id when click on customer*/
 ...
 public function Customer_Info(Request $request)
 {
  ...
  $carts=Cart::where('created_by',$user_id)->where('global_order_id',$global_order_id)->where('order_id',0)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->where('is_split',1)->with('product')->with('cartmodifier')->with('globalorder')->get();
    ...
    $sum_split_product=Cart::where("created_by",$user_id)->where('is_split',1)->where('order_id',0)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->select(DB::raw('sum(base_price * quantity) as total'))->get();

      
      $modifier_total=CartModifier::with('cart')->whereHas('cart',function($query) use($business_session,$branch_session,$table_id_session){
      $query->where('is_split',1)->where("order_id",0)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session);
       })->sum('modifier_price');  
  ...
  public function Show_product(Request $request)
  {
    ...
    $cart_id= $request->cart_id;
    $user_id=Auth::user()->id; 
    $carts=Cart::where('created_by',$user_id)->where("business_id",$business_session)->where("order_id",0)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->where('cart_id',$cart_id)->with('product')->with('cartmodifier')->with('globalorder')->get();
    ...
    $sum_split_product=Cart::where('created_by',$user_id)->where("order_id",0)->where('is_split',1)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->select(DB::raw('sum(base_price * quantity) as total'))->get();
...
/* $Customer_Info=User::where('is_active',1)->where("business_id",$business_session)->where("role_id",5)->get();*/
 ...
 return view('pos-home')->with('categories',$categories)->with('cart_user',$cart_user);
...
/*  $globalOrder_old=Cart::where('created_by',$user_id)->where("order_id",0)->where('created_by',$user_id)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->where('is_split',0)->with('product')->with('cartmodifier')->with('globalorder')->first();
 
  
   if($globalOrder_old->global_order_id!=0||$globalOrder_old->global_order_id!="")

    
     {
          if($globalOrder_old->globalorder->discount!=0||$globalOrder_old->globalorder->discount!="")
             {
             $globalOrder=GlobalOrder::find($globalOrder_old->global_order_id);
             $Order_amt=$globalOrder_old->globalorder->order_amount+$products->price;
             $discount_amount=$globalOrder_old->globalorder->discount*$Order_amt/100;
             $globalOrder->order_amount=$Order_amt;
             $globalOrder->discount_amount= $discount_amount;
             $globalOrder->save();

              $carts= Cart::where("created_by",$user_id)->where("order_id",0)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->update(['global_order_id'=>$globalOrder_old->global_order_id]);

      }

         if($globalOrder_old->globalorder->surcharge!=0||$globalOrder_old->globalorder->surcharge!="")
             {
             $globalOrder=GlobalOrder::find($globalOrder_old->global_order_id);
             $Order_amt=$globalOrder_old->globalorder->order_amount+$products->price;
             $surcharge_amount=$globalOrder_old->globalorder->surcharge*$Order_amt/100;
             $globalOrder->order_amount=$Order_amt;
             $globalOrder->surcharge_amount=$surcharge_amount;
             $globalOrder->save();
...
/*  $categories=Category::where('is_active',1)->where("business_id",$business_session)->where('branch_id',$branch_session)->get();*/
...
/*$carts=Cart::where('user_id',$user_id)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('table_id',$table_id_session)->with('product')->get();
