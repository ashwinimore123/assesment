...
$branch_session = $this->BranchSession();
 ...
 $categories=Category::where('is_active',1)->where("business_id",$business_session)->where
 ('branch_id',$branch_session)->get();
...
$product_id=$request->product_id;
$products=Product::where('is_active',1)->where("business_id",$business_session)->where('branch_id',$branch_session)->where('product_id', $product_id)->first();

$cart=Cart::where("business_id",$business_session)->where('branch_id',$branch_session)->where('product_id',$product_id)->where('table_id',1)->orderBy('cart_id','desc')->first();     
...
$carts=Cart::where('user_id',$user_id)->where("business_id",$business_session)->where('branch_id',$branch_session);
$carts->delete();
