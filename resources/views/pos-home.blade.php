              ...
              <li class="ordersMenu">
                <a class="menuOrders">
                   <div class="animationWrapper menuOrders">Orders
                      <span class="orderCount" id="orderCount">0</span>
                    </div>
                  </a>
               </li>
            </ul>
...
<div class="user_list_scroll scroll-order" style="background: #f8f8f8;">   
<div class="allorders">
...
<div class="postable hidden">
  <div class="posloginheader"> 
    ...
    <a class="menuOrders">
  <div class="animationWrapper menuOrders">Orders
  <span class="orderCount">{{$order_count}}</span>
</div>
</a>
</li>
</ul>
...
<div class="paid_section price_split" style="margin-top:300px;">

<div class="PAID_ITEMS" id="PAID_ITEMS">

<!--  <h1>PAID</h1>
  <div class="form-group">
  <input type="text" class="textfield_split_paid" style="cursor: default;">
...
/*for when we click on price_split wise checkout */
$(document).ready(function(){
$(".split_textbox_boutton").on('click','.btn_chekcout', function(event){ 
 var split_amt=$('.textfield_split').val();
 var split_amt=split_amt.substring(1);
 var total_amt=$('#calculationprice').text();
 var total_amt=total_amt.substring(1);
 alert(total_amt);
 alert(split_amt);
 $(this).parent('div').remove();
     $.ajax({
      url:"{{config('app.baseURL')}}/admin/Split_by_sprice",
      type:'POST',
      data: {'split_amt':split_amt,'total_amt':total_amt,_token:'{{csrf_token()}}'},
      success:function(data){
      
             if(data.paid_transaction_sum=0 || data.paid_transaction_sum==""){
             document.getElementById("totalammount").innerHTML=data.sections;
             ...
             $('.split_textbox_boutton1').empty();
             $('.PAID_ITEMS').empty();
             $('.button_split').trigger('click');
             $('.span_split').remove();
             ...
             document.getElementById("PAID_ITEMS").innerHTML=data.sections1;
             document.getElementById("totalammount").innerHTML=data.sections;
...
/*when we click on header orders*/
$(document).ready(function(){
$(".posorders").on('click','div .menuOrders', function(event){ 
   $('.posorders').addClass('hidden');
         $.ajax({
      url:"{{config('app.baseURL')}}/admin/Add_new_order",
      type:'POST',
...
$(document).ready(function(){
$(".header-right").on('click','li a.sendord', function(event){ 
   
        $.ajax({
        url:"{{config('app.baseURL')}}/admin/check_global_order_id",
        type:'POST',
        data: {_token:'{{csrf_token()}}'},
        success:function(data){
        if (data!="") 
        {
         ...
         else
         {
             ...
             $.ajax({
             url:"{{config('app.baseURL')}}/admin/SendOrder",
             type:'POST',
             data: {_token:'{{csrf_token()}}'}, 
             success:function(data){
             document.getElementById("totalammount").innerHTML=data.sections;
...
/*for when we click on orderback boutton*/ 
$(document).ready(function(){
...
$(".fixed_all").on('click','div .menuOrders', function(event){ 
        ...
        if (data!="") 
        {
         ...
         else
         {
             ...
             success:function(data){
             $('.orderCount').text(data.order_count);
             document.getElementById("ExistingOrders").innerHTML=data.sections;
...
$this attribute($function set ttribute value());
Stored item_id();
if(!isNaN(currentVal));
if(type =='minus') {
...
var n2=document.getElementById("qty_price").innerHTML=n1*n2;
}else if(type == 'plus'){
}
else{
    $('input-number').val();
